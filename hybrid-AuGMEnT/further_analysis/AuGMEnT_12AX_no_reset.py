from AuGMEnT_model_no_reset import AuGMEnT
...
if isinstance(leak, list):
	AuG_type = 'hybrid_AuG'
	reset_fold ='no_reset_hybrid'
	print('Hybrid-AuGMEnT')	 
elif leak!=1:
	AuG_type = 'leaky_AuG' 
	print('Leaky-AuGMEnT')
	reset_fold ='no_reset_leaky'
else:
	AuG_type = 'AuGMEnT' 
...
for n in np.arange(N_sim):
	...
	E[n,:],conv_ep[n],RPE[n,:] = model.training_12AX(N_trial,p_target,criterion,stop,verb,policy,stoc,t_weighted,e_weighted)
...
folder = 'DATA'
str_conv = folder+'/'+reset_fold+'/conv_LONG.txt'
np.savetxt(str_conv,conv_ep)	
E_mean = np.mean(np.reshape(E,(N_sim,-1,50)),axis=2)
str_err = folder+'/'+reset_fold+'/error_LONG.txt'
np.savetxt(str_err,E_mean)
R_mean = np.mean(np.reshape(RPE,(N_sim,-1,50)),axis=2)
str_r = folder+'/'+reset_fold+'/RPE_LONG.txt'
np.savetxt(str_r,R_mean)

if do_test:
	str_perc = folder+'/'+reset_fold+'/perc_expl_LONG.txt'
	np.savetxt(str_perc,perc_expl)
	str_perc = folder+'/'+reset_fold+'/perc_no_expl_LONG.txt'
	np.savetxt(str_perc,perc_no_expl)
	str_perc = folder+'/'+reset_fold+'/perc_soft_LONG.txt'
