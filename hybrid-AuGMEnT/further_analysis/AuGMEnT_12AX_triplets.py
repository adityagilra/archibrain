from AuGMEnT_model_triplets import AuGMEnT
...
folder = 'DATA'
str_conv = folder+'/conv_long_triplets_leaky.txt'
np.savetxt(str_conv,conv_ep)	
E_mean = np.mean(np.reshape(E,(N_sim,-1,50)),axis=2)
str_err = folder+'/error_long_triplets_leaky.txt'
np.savetxt(str_err,E_mean)
R_mean = np.mean(np.reshape(REW,(N_sim,-1,50)),axis=2)
str_r = folder+'/reward_long_triplets_leaky.txt'
np.savetxt(str_r,R_mean)

if do_test:
	str_perc = folder+'/perc_expl_long_triplets_leaky.txt'
	np.savetxt(str_perc,perc_expl)
	str_perc = folder+'/perc_no_expl_long_triplets_leaky.txt'
	np.savetxt(str_perc,perc_no_expl)
	str_perc = folder+'/perc_soft_long_triplets_leaky.txt'
