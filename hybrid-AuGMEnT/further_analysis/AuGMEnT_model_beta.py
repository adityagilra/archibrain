...
class AuGMEnT():
	...
	def compute_response(self, Qvec, policy='eps_greedy',stoc='unif',t_weighted=False,e_weighted=False,bet=10,it=None):
			...
			if np.random.random()<=(1-eps):
				...
				if stoc=='soft':
					if t_weighted==True and it is not None:
						g = 1 + (bet/np.pi)*np.arctan(it/2000)
					else:
						g = 1
...
######### TRAINING FOR SEQUENCE PREDICTION TASK (prediction of final letter only)
	...
	def training_seq_pred(self,N_trial,d,stop=True,verb=True,policy='eps_greedy',stoc='soft',t_weighted=True,bet=10):
		...
		for tr in np.arange(N_trial):
			...
			for i in np.arange(np.shape(S)[0]):
				...
				resp_ind,P_vec = self.compute_response(Q,policy,stoc,t_weighted,bet)
				q = Q[0,resp_ind]
...
######### TRAINING FOR SEQUENCE PREDICTION TASK (CPT variant=response to each cue)
	...
	def training_seq_pred_CPT(self,N_trial,d,bet=10,stop=True,verb=True):
		...
		for tr in np.arange(N_trial):
			...
			for i in np.arange(np.shape(S)[0]):
				...
				resp_ind,P_vec = self.compute_response(Q,policy,stoc,t_weighted,e_weighted,bet)
				q = Q[0,resp_ind]
				z = np.zeros(np.shape(Q))
				z[0,resp_ind] = 1
...
######### TRAINING + TEST FOR SACCADE TASK 
	...
	def training_saccade(self,N_trial,S_train,O_train,reset_case,policy='eps_greedy',stoc='unif',t_weighted=False,e_weighted=False,bet=10,verbose=False,shape_factor=0.2,stop=True):
			...
			if abort==True and jump!=0:
				...
				if phase=='fix' and num_fix<2 and attempts==2:
					while num_fix<2 and attempts<10:
						r,q_old,fix = self.try_again(s_inst,zerozero,o_print,r,q_old,phase,policy,stoc,t_weighted,bet)
						attempts += 1
						if fix==False:
							num_fix = 0
						else:
	...
	def try_again(self,s_i,s_t,o_print,r,q_old,phase,policy='eps_greedy',stoc='unif',t_weighted=False,e_weighted=False,bet=10):
	...
	def test_saccade(self,S_test,O_test,reset_case,policy='eps_greedy',stoc='unif',t_weighted=False,e_weighted=False,bet=10):
...
######### TRAINING + TEST FOR 12AX TASK 
	...
	def training_12AX(self,N_trial,p_target,criterion='strong',stop=True,verb=False,policy='eps_greedy',stoc='unif',t_weighted=False,e_weighted=False,bet=10):
		...
		for tr in np.arange(N_trial):
			...
			for n in np.arange(N_stimuli):	
				...
				resp_ind,_ = self.compute_response(Q,policy,stoc,t_weighted,e_weighted,bet,tr)
				q = Q[0,resp_ind]
	...
	# training variant for given training datasets
	def training_12AX_2(self,S_train,O_train,policy='eps_greedy',stoc='unif',t_weighted=False,e_weighted=False,bet=10):
    ...
    # training variant for given training datasets
	def training_12AX_3(self,S_train,O_train,policy='eps_greedy',stoc='unif',t_weighted=False,e_weighted=False,bet=10):
