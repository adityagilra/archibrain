...
def subset_construction(N=500,p_target=0.25):
	np.random.seed(1234)		
...
def data_construction(N=500,p_target=0.5,p_tr=0.8):
	
	N_tr = int(np.around(N*p_tr))	
	
	# data division in training and test subsets
	[S_tr, O_tr] = subset_construction(N_tr,p_target)
	[S_test, O_test] = subset_construction(N-N_tr,p_target)
