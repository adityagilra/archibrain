...
def data_construction(N=500,perc_training=0.8):
	...
	S_train = np.zeros((idx,6,4))
	O_train = np.zeros((idx,6,6))
	for i,tr in enumerate(Trials_training):
		S_train[i,:,:], O_train[i,:,:] =  build_trial(tr)

	if perc_training!=1:
		Trials_test = Trials[idx:,0] 
		S_test = np.zeros((N-idx,6,4))
		O_test = np.zeros((N-idx,6,6))
...
def main():
	tr = np.random.choice(np.arange(4), 1)
	S_train, O_train = build_trial(tr)
	print(S_train)
	print(O_train)
