## HIERARCHICAL AuGMEnT (DEEP, SELECTION)
...
##
## N.B. The current version of hierarchical model presents a gating mechanism that discriminates in a multiplicative way how to distribute the input information over the different levels. Gating is of type: inp(t)= leak*inp(t-1) + g(t)*s(t)V, where g(t) is a one-hot vector computed in the controller branch according to the probability distribution over the memory levels obtained after softmax activation. This is done to divide the information over the memory levels to not have interference and encouraging level-specialization.
##
...
class hierarchical_AuGMEnT(AuGMEnT):
	...
	def update_tags(self,s_inst,s_trans,y_r,y_m,y_h,g,l_sel,z,resp_ind):

		# memory branch
		g_sel = np.zeros((self.L,1,1))
		g_sel[l_sel,0,0] = g[l_sel,0]
		self.sTRACE = self.sTRACE*self.LEAK + g_sel*np.tile(np.transpose(s_trans), (self.L,1,self.M))
		...
		# gate tag update
		g_sel = np.reshape(g_sel,(1,self.L))
		self.Tag_w_g += -np.reshape(self.ALPHA,(1,self.L))*self.Tag_w_g + np.dot(np.transpose(s_inst), self.g_strength*g_sel*(1-g_sel)*np.transpose(np.sum(np.transpose(np.dot(s_trans,self.V_m),(1,0,2))*delta_m,axis=2)))

	def select_level(self,g):

		l = np.random.choice(np.arange(self.L),1,p=np.squeeze(g,axis=1))
	...
	def training(self,N_ep,p_c,max_loop,conv_criterion='strong',stop=True,verb=False):
		...
		for n_ep in np.arange(N_ep):
			...
			for n in np.arange(N_stimuli):
				...
				y_r,y_m,y_h,g,l_sel,Q = self.feedforward(s_inst, s_trans)
				...
				self.update_tags(s_inst,s_trans,y_r,y_m,y_h,g,l_sel,z,resp_ind)
...
if (task_selection!="2"):
	...
	M = 3			     # dimension of the memory units
	H = 10
	...
	eps = 0.1			# fraction of softmax modality for activity selection
