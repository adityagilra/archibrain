...
##
## N.B. The current version of gated model presents a gating mechanism that discriminates in a multiplicative way how to store the input information. Gating is of type: inp(t)= lf(t)*inp(t-1) + g(t)*s(t)V, where g(t) and f(t) are scalars computed in the controller branch after hard-sigmoid activation.
##
...
class gated_AuGMEnT(AuGMEnT):
	...
	def initialize_weights_and_tags(self):
		...
		self.W_f = range*np.random.random((self.S, self.M)) - range/2
	...
	def reset_tags(self):
		...
		self.Tag_w_f = 1e-6*np.ones((self.S, self.M))
	...
	def update_tags(self,s_inst,s_trans,y_r,y_m,g,f,z,resp_ind):

		# memory branch
		self.sTRACE = self.sTRACE*f + np.dot(np.transpose(s_trans), g)
		...
		# gate tag update
		self.Tag_w_g += -self.alpha*self.Tag_w_g + np.dot(np.transpose(s_inst), self.g_strength*g*(1-g)*np.dot(s_trans,self.V_m)*y_m*(1-y_m)*self.W_m_back[resp_ind,:])
		self.Tag_w_f += -self.alpha*self.Tag_w_f + np.dot(np.transpose(s_inst), self.g_strength*g*(1-g)*self.cumulative_memory*y_m*(1-y_m)*self.W_m_back[resp_ind,:])
	...
	def feedforward(self,s_inst,s_trans):
		...
		y_m, self.cumulative_memory = act.sigmoid_gated(s_trans, self.V_m, self.cumulative_memory, f, g)
	...
	def training(self,N_ep,p_c,max_loop,conv_criterion='strong',stop=True,verb=False):
		...
		for n_ep in np.arange(N_ep):
			...
			for n in np.arange(N_stimuli):
				...
				self.update_tags(s_inst,s_trans,y_r,y_m,g,f,z,resp_ind)
