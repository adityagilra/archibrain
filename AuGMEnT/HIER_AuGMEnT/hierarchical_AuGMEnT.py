...
class hierarchical_AuGMEnT(AuGMEnT):
	...
	def update_weights(self,RPE):
		...
		self.W_g += self.BETA[:,0,0]*RPE*self.Tag_w_g
	...
	def update_tags(self,s_inst,s_trans,y_r,y_m,g,z,resp_ind):
		...
		self.sTRACE = self.sTRACE*self.LEAK + np.tile(np.transpose(s_trans), (self.L,1,self.M))
		...
		self.Tag_w_g += -self.ALPHA[:,0,0]*self.Tag_w_g + np.dot(np.transpose(s_inst), self.g_strength*g_exp*(1-g_exp)*np.sum(np.dot(s_trans,self.V_m)*np.transpose(y_m*(1-y_m),(1,0,2))*self.W_m_back[:,resp_ind,:],axis=2))
