...
##
## N.B. The current version of gated model presents a gating mechanism that discriminates in a multiplicative way how to store the input information. Gating is of type: inp(t)= leak*inp(t-1) + g(t)*s(t)V, where g(t) is a scalar computed in the controller branch after hard-sigmoid activation.
##
...
class gated_AuGMEnT(AuGMEnT):
	...
	def __init__(self,S,R,M,A,alpha,beta,discount,leak,eps,gain,rew_rule='RL',dic_stim=None,dic_resp=None,prop='std',g_str=1):
		...
		super(gated_AuGMEnT,self).__init__(S,R,M,A,alpha,beta,discount,eps,gain,leak,rew_rule,dic_stim,dic_resp,prop)
	...
	def update_tags(self,s_inst,s_trans,y_r,y_m,g,z,resp_ind):
		...
		# memory branch
		self.sTRACE = self.sTRACE*self.memory_leak + np.dot(np.transpose(s_trans), g)
	...
	def feedforward(self,s_inst,s_trans):
		...
		y_m, self.cumulative_memory = act.sigmoid_gated(s_trans, self.V_m, self.cumulative_memory, self.memory_leak, g)
...
if (task_selection!="2"):
	...
	if do_training:
		...
		for n in np.arange(N_sim):
			...
			model = gated_AuGMEnT(S,R,M,A,alpha,beta,discount,leak,eps,g,rew,dic_stim,dic_resp,prop,g_str)
