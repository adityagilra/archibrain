...
##
## N.B. The current version of hierarchical model presents a gating mechanism that discriminates in a multiplicative way how to distribute the input information over the different levels. Gating is of type: inp(t)= f(t)*inp(t-1) + g(t)*s(t)V, where g(t) amd f(t) are vector computed in the controller branch after softmax (for g) and hard-sigmoid (for f) activations where each unit is associated to a memory level.
##
...
class hierarchical_AuGMEnT(AuGMEnT):
	...
	def __init__(self,L,S,R,M,A,ALPHA,BETA,discount,eps,gain,rew_rule='RL',dic_stim=None,dic_resp=None,prop='std'):
	...
	def initialize_weights_and_tags(self):
		...
		self.W_f = np.random.random((self.S, self.L)) - 0.5
	...
	def update_weights(self,RPE):
		...
		self.W_f += self.beta*RPE*self.Tag_w_f
	...
	def reset_tags(self):
		...
		self.Tag_w_f = 1e-6*np.ones((self.S, self.L))
	...
	def compute_response(self, Qvec, it=None):
		...
		if np.random.random()<=(1-self.epsilon):
			...
			if it is not None:
				g = self.gain*(1+ (8/np.pi)*np.arctan(it/1000))
			else:
				g = self.gain
		...
		for n_ep in np.arange(N_ep):
			...
			for n in np.arange(N_stimuli):
				...
				if verbose:
					...
					print('\t\tG: ',np.transpose(g),'\tF_ ',np.transpose(f))
...
if (task_selection!="2"):
	...
	model = hierarchical_AuGMEnT(L,S,R,M,A,ALPHA,BETA,discount,eps,g,rew,dic_stim,dic_resp,prop)
	...
	if do_training:
		...
		E,conv_ep = model.training(N,p_c,'strong',True,verb)
