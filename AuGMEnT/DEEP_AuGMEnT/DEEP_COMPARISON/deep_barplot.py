...
str_AUG_DC =  'DC_conv_3.txt'
X_AUG_DC = np.loadtxt(str_AUG_DC)
str_AUG_DM =  'DM_conv_3.txt'
X_AUG_DM = np.loadtxt(str_AUG_DM)
str_AUG_DMC =  'DCM_conv_3.txt'
X_AUG_DMC = np.loadtxt(str_AUG_DMC)
#str_HER = 'HER_12AX_conv.txt'
#X_HER = np.loadtxt(str_HER)

X = np.concatenate([X_AUG_l,X_AUG_DC,X_AUG_DM,X_AUG_DMC])
N_sim = np.shape(X_AUG_l)[0]
X = np.transpose(np.reshape(X,(-1,N_sim)))

print(np.shape(X))

LABELS = ['Leaky-AUG', 'Deep-AuG\n(DC)', 'Deep-AuG\n(DM)', 'Deep-AuG\n(DMC)']
...
colors = ['#46A375','#3f703f','#2A612A','#214d21']
