...
for n in np.arange(N_sim):
	...
	E_fix[n,:],E_go[n,:],conv_ep[n] = model.training_saccade(N_trial,S_tr,O_tr,reset_cond,verb,shape_fac,stop)
...
str_go = folder+'/AuGMEnT_long_'+task+'_error_go.txt'
np.savetxt(str_go,E_go_mean)	
E_fix_mean = np.mean(np.reshape(E_fix,(-1,50)),axis=1)
str_fix = folder+'/AuGMEnT_long_'+task+'_error_fix.txt'
np.savetxt(str_fix,E_fix_mean)
str_perc_go = folder+'/AuGMEnT_long_'+task+'_perc_go.txt'
np.savetxt(str_perc_go,perc_go)	
str_perc_fix = folder+'/AuGMEnT_long_'+task+'_perc_fix.txt'
