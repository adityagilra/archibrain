...
def construct_trial(dist=2,p=0.5):
	...
	for i in np.arange(time_length-1)+1:
		S_tr[i,i]=1
	O_tr = np.zeros((time_length,num_letters))
	for i in np.arange(time_length-1):
		O_tr[i,i]=1
	...
	if x==0:
		S_tr[0,0]=1
		O_tr[time_length-1,num_letters-2]=1
	else:
		S_tr[0,num_letters-1]=1
		O_tr[time_length-1,num_letters-1]=1
...
def main():
	...
	print(S)
	print(O)
	print('INP: ',d_s[np.argmax(S[0,:])],'\t OUT: ',d_o[np.argmax(O[d,:])])
