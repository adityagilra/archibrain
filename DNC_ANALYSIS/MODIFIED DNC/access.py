# MODIFICATION OF access.py of original DNC from https://github.com/deepmind/dnc.
# Changements to the original files are restricted to two points:
# - Simplification of addressing scheme to pure content-based approach, both for writing and reading from memory (location-based steps or temporal linkage are commented)
# - LRUA module (from A. Santoro, 'One-shot Learning with Memory-Augmented Neural Networks', 2016) is an additional option for writing operation via lrua_cond variable(default is True)
#
  ...
  *   `addressing.FreenessAllocator` for keeping track of memory usage, where
  ...
  def _build(self, inputs, prev_state):
    ...
    if self.LRUA==False:
        ...
        write_weights, _ = self._write_weights_LRUA(inputs, prev_state.read_weights, usage)
        #erase_all = tf.ones_like(inputs['erase_vectors'],dtype=inputs['erase_vectors'].dtype)
        #memory = _erase_and_write(prev_state.memory,address=least_usage_weights,reset_weights=erase_all,values=inputs['write_vectors'])
  ...
  def _write_weights(self, inputs, memory, usage):
    ...
    with tf.name_scope('write_weights', values=[inputs,memory,usage]):
      ...
      return write_gate * write_content_weights
      
  def _write_weights_LRUA(self, inputs, prev_read, usage):
    """Calculates the memory locations to write to.
    ...
    Returns:
      tensor of shape `[batch_size, num_writes, memory_size]` indicating where
          to write to (if anywhere) for each write head.
    """
    with tf.name_scope('write_weights_LRUA', values=[inputs,prev_read,usage]):
      
      # c_t^{w, i} - The content-based weights for each write head.
      if self._num_writes==1:
      	write_content_weights = tf.reduce_sum(prev_read,axis=1,keep_dims=True)
      else:
        write_content_weights = prev_read

      # lu_t^i - The least usage locations in memory
      _,least_usage_positions = tf.nn.top_k(input=-usage,k=self._num_writes)
      least_usage_weights = tf.one_hot(indices=least_usage_positions,depth=usage.get_shape().as_list()[1], dtype=write_content_weights.dtype)
      ...
      # w_t^{w, i} - The write weightings for each write head.
      return write_gate * (allocation_gate * write_content_weights +
                           (1 - allocation_gate) * least_usage_weights ), least_usage_weights
