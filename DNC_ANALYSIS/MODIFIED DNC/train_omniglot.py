...
#  
# TASK: image classification task on Omniglot dataset
#
...
tf.flags.DEFINE_float("rot_max", 30, "Maximum absolute rotation of sample images.")
tf.flags.DEFINE_float("shift_max", 10, "Maximum absolute shift of sample images.")

# Training options.
tf.flags.DEFINE_integer("num_episodes", 100000, "Dimension of the training dataset")
...
def run_model(input_sequence, output_size):
  ...
  dnc_core = dnc.DNC(access_config, controller_config, output_size, clip_value, FLAGS.conc_opt)
...
def train(num_trials, report_interval):
  ...
  generator = OmniglotGenerator(data_folder='./data/omniglot', batch_size=FLAGS.batch_size, nb_samples=FLAGS.nb_class, nb_samples_per_class=FLAGS.nb_samples_per_class, max_rotation=FLAGS.rot_max, max_shift=FLAGS.shift_max, max_iter=FLAGS.num_episodes)  
  ...
  optimizer = tf.train.AdamOptimizer(FLAGS.learning_rate, epsilon=FLAGS.optimizer_epsilon)
  ...
  with tf.train.SingularMonitoredSession(hooks=hooks, checkpoint_dir=FLAGS.checkpoint_dir) as sess:
	...
	print('Omniglot Task - R-DNC - conc_input -',FLAGS.hidden_size,'hidden - ',FLAGS.learning_rate,'lr')
    ...
    for i, (batch_input, batch_output) in generator:
      ...
      _, loss, acc, lab_distr = sess.run([train_step, train_loss, accuracies, sum_out], feed_dict=feed_dict)
...
def main(unused_argv):
  ...
  train(FLAGS.num_episodes, FLAGS.report_interval)
