...
class OmniglotGenerator(object):
    ...
    def sample(self, nb_samples):
        ...
        example_inputs = np.zeros(( nb_samples * self.nb_samples_per_class, self.batch_size, np.prod(self.img_size)), dtype=np.float32)
        example_outputs = np.zeros((nb_samples * self.nb_samples_per_class, self.batch_size), dtype=np.float32)     #notice hardcoded np.float32 here and above, change it to something else in tf
